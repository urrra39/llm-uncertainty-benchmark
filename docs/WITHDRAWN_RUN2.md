# Withdrawn: run #2's tables, findings and narrative

> **Status: WITHDRAWN.** Everything below this line describes run #2
> (`configs/run2.yaml`, `results_run2_withdrawn.json`), whose ranking was
> withdrawn because up to 34 of its 120 labels (28%) could flip under the
> echo-contamination bound. Nothing below may be cited as a finding of this
> repository. The text is preserved verbatim as the record, except that
> present-tense claims carry an explicit status marker. The primary result is
> run #2b's (see README.md).

> Run #2 opened with the paragraphs below (preserved verbatim).
...
**1. (Withdrawn.) No signal had separated from the others at n=120.** Run #2 measured: The top six span AUROC
...
**2. (Withdrawn.) A trivial question-length baseline had nearly matched 6×-cost
self-consistency, and the gap was not significant.** Run #2 measured: `t_question_length` — the
...
**3. (Withdrawn.) Self-consistency had saturated at N=3; the last two samples were wasted.** Run #2 measured:
...
**4. (Withdrawn.) The per-dataset split had changed the ranking, so the pooled table was
flattered by the dataset mix.** Run #2 measured: PopQA (n=90) is 40% incorrect; TriviaQA (n=30)
...
The practical conclusion run #2 drew, argued in full under
["What I would ship"](#what-i-would-ship): that run did not establish any
...
The primary table, as run #2 published it (withdrawn). Run #2, n=120, `configs/run2.yaml`, positive class
...
Every number below was computed in run #2 (withdrawn) on the identical 120-row frozen
...
### N-ablation (withdrawn): self-consistency had saturated at N=3 in run #2
...
In run #2 (withdrawn), ECE on held-out rows only, 10 bins, Platt map fitted on the train split alone
...
In run #2 (withdrawn), 5-fold CV logistic regression over `b_distinct_count` + `b_disagreement_rate`
...
In run #2 (withdrawn): 0 parse failures across 120 rows, 4 distinct values
...
> Superseded planning note (kept verbatim below): two claims in this
> subsection are stale. `generation_batch_size` is pinned at 1 because D27
> was REOPENED, not because uniform batches are proven identical on CUDA.
> And run #2's findings below WERE weakened after all — by the echo bound —
> after this paragraph was written. The live plan is `configs/run3_gpu.yaml`
...
**Nothing, on run #2's evidence (withdrawn).** That run did not support shipping any of
